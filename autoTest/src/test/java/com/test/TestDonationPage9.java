...
public class TestDonationPage9 {
    ...
    public static void testDonationLink() throws InterruptedException {
        ...
        LoginDriverSetUp.loginSetup(driver, usernameDonor, passwordDonor);

        EventPageDriverSetUp.clickDonationLink(driver);
        driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        assert driver.getCurrentUrl().equals("http://localhost:8084/donation");

        DonationPage donationPage = new DonationPage(driver);
        donationPage.clickUpdateFirst();

        //test update
        DonationUpdatePage donationUpdatePage = new DonationUpdatePage(driver);
        Integer updateVolume = 20;
        Integer updateZip = 52240;
        donationUpdatePage.sendDonationVolume(updateVolume);
        donationUpdatePage.sendZip(updateZip);
        donationUpdatePage.setSaveDonation();
        assert driver.getCurrentUrl().equals("http://localhost:8084/donation");

        //test delete
        donationPage.clickDeleteFirst();
        assert driver.getCurrentUrl().equals("http://localhost:8084/donation");
